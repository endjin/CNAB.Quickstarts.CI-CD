...
function parse(imageName) {
  ...
  var pathParts; // my-registry.local/foo/bar

  if (imageParts.length >= 3) {
    result.domain = imageParts[0];
    pathParts = imageParts.slice(1, imageParts.length);
  } else {
    pathParts = imageParts;
  } // parse tag/digest


  var lastPart = pathParts[pathParts.length - 1];
  var re = /^(?<name>[a-zA-Z09_\\.-]+)(?::(?<tag>\w[a-zA-Z0-9_\\.-]*))?(?:@(?<digest>sha256:[a-zA-Z0-9]+))?$/;
  var lastPartMatches = re.exec(lastPart);

  if (!lastPartMatches || !lastPartMatches.groups || !lastPartMatches.groups.name) {
    throw new Error("Could not parse image name");
  }

  pathParts[pathParts.length - 1] = lastPartMatches.groups.name;
  result.path = pathParts.join("/");

  if (lastPartMatches.groups.tag) {
    result.tag = lastPartMatches.groups.tag;
  }

  if (lastPartMatches.groups.digest) {
    result.digest = lastPartMatches.groups.digest;
  }
